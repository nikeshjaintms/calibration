    ...
    <div class="page-inner">
        ...
        <div class="card">
            <div class="card-header">
                <div class="card-title">Edit Calibration</div>
            </div>

            <form method="POST" action="{{ route('calibrations.update', $calibration->id) }}" id="calibrationForm">
                @csrf
                @method('PUT')
                    ...
                    <div class="row">
                            ...
                            <div class="form-group">
                                ...
                                <select name="jobcard_id" id="jobcard_id" class="form-select" required>
                                    ...
                                    <option value="{{ $jobcard->id }}" 
                                        ...
                                        {{ old('jobcard_id', $calibration->jobcard_id) == $jobcard->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select name="user_id" class="form-select select2">
                                    ...
                                    <option value="{{ $user->id }}" {{ old('user_id', $calibration->user_id) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                                    @endforeach
                                </select>
                                @error('user_id')
                                <span class="text-danger">{{ $message }}</span>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="date" name="date" class="form-control" value="{{ old('date', $calibration->date) }}" required>
                                @error('date')
                                <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-4 mt-3">
                            <div class="form-group">
                                ...
                                <input type="text" name="instrument" class="form-control" placeholder="e.g. Master Pressure Gauge" value="{{ old('instrument', $calibration->instrument) }}" required>
                                @error('instrument')
                                <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-4 mt-3">
                            <div class="form-group">
                                ...
                                <input type="text" name="certificate_number" class="form-control" placeholder="e.g. CERT/2026/001" value="{{ old('certificate_number', $calibration->certificate_number) }}" required>
                        ...
                        <div class="col-md-2 mt-3">
                            <div class="form-group">
                                <label>Temp (°C)</label>
                                <input type="text" name="temperature" class="form-control" placeholder="25" value="{{ old('temperature', $calibration->temperature) }}">
                            </div>
                        </div>
                        ...
                        <div class="col-md-2 mt-3">
                            <div class="form-group">
                                ...
                                <input type="text" name="humidity" class="form-control" placeholder="50" value="{{ old('humidity', $calibration->humidity) }}">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="table-responsive">
                                <table class="table table-bordered" id="pointsTable">
                                    ...
                                    <tbody>
                                        @php
                                            $points = old('points', $calibration->points);
                                        @endphp
                                        @foreach($points as $index => $point)
                                        <tr>
                                            <td><input type="text" name="points[{{ $index }}][set_point_percentage]" class="form-control set-point-pct" value="{{ is_array($point) ? $point['set_point_percentage'] : $point->set_point_percentage }}" readonly></td>
                                            <td><input type="number" step="0.01" name="points[{{ $index }}][expected]" class="form-control expected-val" value="{{ is_array($point) ? $point['expected'] : $point->expected }}" required readonly></td>
                                            <td><input type="number" step="0.01" name="points[{{ $index }}][as_found]" class="form-control found-val" value="{{ is_array($point) ? $point['as_found'] : $point->as_found }}"></td>
                                            <td><input type="number" step="0.01" name="points[{{ $index }}][as_left]" class="form-control left-val" value="{{ is_array($point) ? $point['as_left'] : $point->as_left }}"></td>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select name="result" class="form-select" required>
                                    <option value="pass" {{ old('result', $calibration->result) == 'pass' ? 'selected' : '' }}>PASS</option>
                                    <option value="fail" {{ old('result', $calibration->result) == 'fail' ? 'selected' : '' }}>FAIL</option>
                                </select>
                            </div>
                        </div>
                    </div>
...
<script>
    $(document).ready(function() {
        // Remove manual row controls as points are fixed
