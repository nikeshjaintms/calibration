...
<head>
    ...
    <title>Inspection & Oil Filling Report - {{ $jobcard->jobcard_number }}</title>
    <style>
        body {
...
<body>
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="certificate-title">INSPECTION & OIL FILLING REPORT</div>
        </div>
        ...
        @if ($oil_filling)
            <div class="section-header">OIL FILLING DETAILS</div>
            <div class="row">
                <div class="col-50">
                    <table class="info-table">
                        <tr>
                            <td class="info-label">Oil Type:</td>
                            <td>{{ $oil_filling->oil_type }}</td>
                        </tr>
                        <tr>
                            <td class="info-label">Filling Date:</td>
                            <td>{{ \Carbon\Carbon::parse($oil_filling->filling_date)->format('d/m/Y') }}</td>
                        </tr>
                    </table>
                </div>
                <div class="col-50">
                    <table class="info-table">
                        <tr>
                            ...
                            <td>{{ $oil_filling->quantity }}</td>
                        </tr>
                        <tr>
                            <td class="info-label">Filled By:</td>
                            <td>{{ $oil_filling->user->name ?? $oil_filling->filled_by ?? 'N/A' }}</td>
                        </tr>
                    </table>
                </div>
            </div>

            <div class="section-header">DIAPHRAGM / ACCESSORIES</div>

            <table class="info-table">
                <tr>
                    <td class="info-label">MOC :</td>
                    <td>{{ $oil_filling->moc->name ?? 'N/A' }}</td>
                    <td class="info-label">FLANGE :</td>
                    <td>{{ $oil_filling->flange->name ?? 'N/A' }}</td>
                </tr>
                <tr>
                    <td class="info-label">CAPILLARY :</td>
                    <td colspan="3">{{ $oil_filling->capillary->name ?? 'N/A' }}</td>
                </tr>
            </table>
        @endif
